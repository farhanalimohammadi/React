...
export default function Sidebar(){
    ...
    return(
                <div className={`flex flex-col justify-start items-center bg-componentBg-mainBg text-textsColor-texts
                 h-full fixed left-XP overflow ${showScroll ? "top-topSpace " : "top-3"} w-[270px] rounded-xl transition-all duration-300 ease-linear pb-8 `}>
                    ...
                    border-solid ">
                        {sideLinks.map((item) => (
                            item.lengthh === 2 ? (
                                <Link to={item.link} key={item.id} className="group flex flex-row justify-start items-center w-full gap-x-[8px] py-4 cursor-pointer transition-all duration-300">
